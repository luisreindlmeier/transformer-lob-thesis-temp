...
def bootstrap_confidence_interval(y_true: np.ndarray, y_pred: np.ndarray, metric_fn=None,
                                   n_bootstrap: int = 1000, confidence_level: float = 0.95, seed: int = 42) -> Tuple[float, float, float]:
...
def create_significance_report(y_true: np.ndarray, predictions: Dict[str, np.ndarray], output_path: Optional[str] = None) -> str:
