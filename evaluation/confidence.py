"""Calibration metrics, temperature scaling, and backtesting."""
import json
...
class TemperatureScaler(nn.Module):
    ...
    def set_temperature(self, val_loader: DataLoader, max_iter: int = 50, lr: float = 0.01) -> float:
        self.model.eval()
...
def optimal_confidence_threshold(y_true: np.ndarray, y_pred: np.ndarray, y_prob: np.ndarray, min_coverage: float = 0.1):
    confidences = np.max(y_prob, axis=1)
...
def strategy_returns(y_pred: np.ndarray, y_ret: np.ndarray) -> np.ndarray:
    ...
    strat_ret[y_pred == 2] = y_ret[y_pred == 2]
    strat_ret[y_pred == 0] = -y_ret[y_pred == 0]
    return strat_ret


def filtered_strategy_analysis(y_true: np.ndarray, y_pred: np.ndarray, y_prob: np.ndarray, y_ret: np.ndarray,
                               confidence_thresholds: np.ndarray | None = None) -> dict:
...
def analyze_confidence(model: torch.nn.Module, val_loader: DataLoader, test_loader: DataLoader,
                       y_ret_test: np.ndarray | None = None, output_dir: Path | None = None,
    ...
    _plot_reliability_diagram(all_labels, probs_cal, model_name, plot_path)
...
def _plot_reliability_diagram(y_true: np.ndarray, y_prob: np.ndarray, model_name: str, save_path: Path, n_bins: int = 10) -> None:
    data = reliability_diagram_data(y_true, y_prob, n_bins)
