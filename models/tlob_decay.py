"""TLOB with Decay Attention."""
import torch
from torch import nn
from einops import rearrange
from models.tlob import BiN, MLP, sinusoidal_positional_embedding
from models.decay_attention import DecayAttention


class DecayTransformerLayer(nn.Module):
    def __init__(self, hidden_dim: int, num_heads: int, final_dim: int, max_seq_len: int, init_decay: float = 0.1):
        super().__init__()
...
class TLOBDecay(nn.Module):
    def __init__(self, hidden_dim: int, num_layers: int, seq_size: int, num_features: int, num_heads: int,
                 is_sin_emb: bool = True, dataset_type: str = "LOBSTER", init_decay: float = 0.1, device: str = "cpu"):
        super().__init__()
        ...
        for i in range(num_layers):
            if i != num_layers - 1:
                self.layers.append(DecayTransformerLayer(hidden_dim, num_heads, hidden_dim, seq_size, init_decay))
                self.layers.append(DecayTransformerLayer(seq_size, num_heads, seq_size, hidden_dim, init_decay))
            else:
                self.layers.append(DecayTransformerLayer(hidden_dim, num_heads, hidden_dim // 4, seq_size, init_decay))
                self.layers.append(DecayTransformerLayer(seq_size, num_heads, seq_size // 4, hidden_dim // 4, init_decay))
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if self.dataset_type == "LOBSTER" and input.shape[-1] > 41:
