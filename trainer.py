"""Lightning Module for TLOB training with EMA and manual LR scheduling."""
import os
...
class TLOBTrainer(LightningModule):
    def __init__(self, seq_size: int = cfg.SEQ_SIZE, hidden_dim: int = cfg.HIDDEN_DIM,
        ...
        # EMA with decay=0.999 (critical for reproducibility)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # Manual LR scheduling (halve on small/no improvement)
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> torch.Tensor:
        x, y = batch
        # Use EMA only in TRAINING mode; in EVALUATION, checkpoint already has EMA params
